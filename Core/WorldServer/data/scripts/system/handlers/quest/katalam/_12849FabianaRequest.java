 ...
 * @author Romanz
 */
public class _12849FabianaRequest extends QuestHandler {

    private final static int questId = 12849;

    public _12849FabianaRequest() {
        super(questId);
    }
    ...
    public boolean onKillInWorldEvent(QuestEnv env) {
        ...
        if (env.getVisibleObject() instanceof Player && player != null && player.isInsideZone(ZoneName.get("TORN_LAND_600050000"))) {
    ...
    public boolean onEnterZoneEvent(QuestEnv env, ZoneName zoneName) {
        if (zoneName == ZoneName.get("TORN_LAND_600050000")) {
            Player player = env.getPlayer();
            if (player == null) {
                return false;
            }
