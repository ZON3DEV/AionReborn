...
public class CalindiFlamelordAI2 extends AggressiveNpcAI2
{
    ...
    private void blazeEngraving()
    {
        if (Rnd.get(0, 100) < 2 && getPosition().getWorldMapInstance().getNpc(283130) == null)
        {
            ...
            Npc summon = (Npc) spawn(283130, target.getX(), target.getY(), target.getZ(), (byte) 0);
            AI2Engine.getInstance().setupAI("calindisummon", summon);
            ((NpcAI2) summon.getAi2()).setStateIfNot(AIState.IDLE);
        }
    }
    ...
    private void rndSpawn(int npcId, int count)
    {
        for (int i = 0; i < count; i++)
        {
            ...
            Npc summon = (Npc) SpawnEngine.spawnObject(template, getPosition().getInstanceId());
            AI2Engine.getInstance().setupAI("calindisummon", summon);
            ((NpcAI2) summon.getAi2()).setStateIfNot(AIState.IDLE);
        }
    }
