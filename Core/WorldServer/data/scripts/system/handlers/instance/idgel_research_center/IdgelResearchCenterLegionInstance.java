...
public class IdgelResearchCenterLegionInstance extends GeneralInstanceHandler {
    ...
    private int getTime() {
        ...
        } else if (result < 600000) { // 10 Минут
    ...
    private int checkRank(int totalPoints) {
        int rank = 0;
        if (totalPoints > 5000) { // S Ранг.
            rank = 1;
        } else if (totalPoints > 3500) { // A Ранг.
            rank = 2;
        } else if (totalPoints > 2700) { // B Ранг.
            rank = 3;
        } else if (totalPoints > 2200) { // C Ранг.
            rank = 4;
        } else if (totalPoints > 1600) { // D Ранг.
            rank = 5;
        } else if (totalPoints > 500) { // F Ранг.
            rank = 6;
        } else {
            rank = 8;
        }
    ...
    public void onEnterInstance(final Player player) {
        if (instanceTimer == null) {
            ...
            instanceTimer = ThreadPoolManager.getInstance().schedule(new Runnable() {
                ...
                public void run() {
                    openFirstDoors();
                    sendMsg(1401852); //Старт Лаборотории Идгеля
    ...
    public void stopInstanceAfterTime(final Player player) {
        instanceTimer = ThreadPoolManager.getInstance().schedule(new Runnable() {
            ...
            public void run() {
                stopInstance(player);
                spawn(730730, 571.1385f, 444.94006f, 102.65536f, (byte) 30); //Лоборотория Идгеля Выход
            }
        }, 60000); //10 Минут
    }
    ...
    public void doReward(Player player) {
        for (Player p : instance.getPlayersInside()) {
            ...
            if (!instanceReward.isRewarded()) {
                ...
                AbyssPointsService.addAp(player, (int) rewardAp); // Очки Бездны.
                if (instanceReward.getPoints() >= 2700) {
                    instanceReward.setSillus((int) rewardSillus);
                    ItemService.addItem(player, 186000239, (int) rewardSillus); // Эмблемма крепости Силлус.
                    instanceReward.setCeramium((int) rewardCeramium);
                    ItemService.addItem(player, 186000242, (int) rewardCeramium); // Серамиумовая медаль.
                }
                if (instanceReward.getPoints() >= 5000) {
                    instanceReward.setFavorable(1);
                    ItemService.addItem(player, 188052543, 1); // Мешочек с почетной наградой.
                }
            }
        }
    }
