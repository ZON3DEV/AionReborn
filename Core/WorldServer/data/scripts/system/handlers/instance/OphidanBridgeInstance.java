...
public class OphidanBridgeInstance extends GeneralInstanceHandler implements PlayerController.Listener {
    ...
    public void onInstanceCreate(WorldMapInstance instance) {
        ...
        spawn(701155, 435.9603f, 495.93878f, 604.8871f, (byte) 90); // Mur invisible
        spawn(231050, 317f, 489.04999f, 607.64343f, (byte) 0);
		
		spawn(231058, 584f, 396f, 603f, (byte) 61);
		spawn(231058, 575f, 476f, 620f, (byte) 27);
		spawn(231058, 619f, 515f, 592f, (byte) 111);
    }
    ...
    public void onDie(Npc npc) {
        ...
        switch (npc.getNpcId()) {
            case 231050:
                spawn(730868, 350.18478f, 490.73065f, 606.34015f, (byte) 2);
                break;
            case 230413:
                sendMsg(1401904);
            ...
            case 230420:
                ...
                spawn(701155, 435.9603f, 495.93878f, 604.8871f, (byte) 90); // Mur invisible
                spawn(701155, 377.6052f, 492.46472f, 605.38f, (byte) 30); // Mur invisible
    ...
    private void startFinalTimer() {
        sendMsg(1401892);
        this.sendMessage(1401875, 15 * 60 * 1000); //10 минут до выстрела
        this.sendMessage(1401876, 20 * 60 * 1000); //5 минут до выстрела
        this.sendMessage(1401877, 24 * 60 * 1000); //1 минута до выстрела

        failTask = ThreadPoolManager.getInstance().schedule(new Runnable() {
            @Override
            public void run() {
                sendMsg(1401878); //Сообщение о заряженной пушке бомбарда
                final Npc vera = instance.getNpc(231050);//Бомбард
                if (vera != null) {
                    SkillEngine.getInstance().getSkill(vera, 21442, 60, vera).useNoAnimationSkill();/*Бомбануло)))*/
    ...
    public void SpawnRight(Player player) {
        ...
        attacktransformedPlayer(player, (Npc) spawn(231193, (player.getX() + x1), (player.getY() + y1), player.getZ(), (byte) player.getHeading()));
    }
