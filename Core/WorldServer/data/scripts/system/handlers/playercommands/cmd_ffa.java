    ...
    public void execute(final Player player, String... params) {
	    if (player.getLevel() < 37) {
            if (params.length < 1 || params == null) {
                onFail(player, null);
				PacketSendUtility.sendSys1Message(player, "\uE005", "You must reached lvl 37 or better for use <FFA> !!!");
                return;
			}
        }
        ...
        if (params[0].equals("enter")) {
			...
			PacketSendUtility.sendSys1Message(player, "\uE005", "You will be ported to FFA after 10 Seconds.");
        ...
        if (params[0].equals("leave")) {
            ...
            player.getEffectController().updatePlayerEffectIcons();
            player.getEffectController().broadCastEffects();
			PacketSendUtility.sendSys1Message(player, "\uE005", "You will be ported out of FFA in 10 Seconds.");
            ThreadPoolManager.getInstance().schedule(new Runnable() {
                @Override
                public void run() {
                    player.getEffectController().unsetAbnormal(AbnormalState.SLEEP.getId());
