...
public class MySQL5PlayerDAO extends PlayerDAO {
	...
	public void storePlayer(final Player player) {
		...
		try {
			con = DatabaseFactory.getConnection();
			PreparedStatement stmt = con.prepareStatement("UPDATE players SET name=?, exp=?, recoverexp=?, x=?, y=?, z=?, heading=?, world_id=?, gender=?, race=?, player_class=?, last_online=?, quest_expands=?, npc_expands=?, advanced_stigma_slot_size=?, wh_npc_expands=?, note=?, title_id=?, bonus_title_id=?, dp=?, soul_sickness=?, mailbox_letters=?, reposte_energy=?, bg_points=?, mentor_flag_time=?, initial_gamestats=?, world_owner=? WHERE id=?");
	...
	public boolean saveNewPlayer(final PlayerCommonData pcd, final int accountId, final String accountName) {
		...
		try {
			con = DatabaseFactory.getConnection();
			PreparedStatement preparedStatement = con.prepareStatement("INSERT INTO players(id, `name`, account_id, account_name, x, y, z, heading, world_id, gender, race, player_class , quest_expands, npc_expands, wh_npc_expands, online) " + "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 0)");
			...
			preparedStatement.setInt(15, pcd.getWhNpcExpands());
	...
	public PlayerCommonData loadPlayerCommonData(final int playerObjId) {
		...
		try {
			...
			if (resultSet.next()) {
				...
				if (DataManager.WORLD_MAPS_DATA.getTemplate(worldId).isInstance()) {
					if (World.getInstance().getWorldMap(worldId) == null || World.getInstance().getWorldMap(worldId).getMainWorldMapInstance() == null) {
						InstanceExit instanceExit = DataManager.INSTANCE_EXIT_DATA.getInstanceExit(worldId, cd.getRace());
						if (instanceExit == null) {
							DataManager.PLAYER_INITIAL_DATA.getSpawnLocation(cd.getRace());
