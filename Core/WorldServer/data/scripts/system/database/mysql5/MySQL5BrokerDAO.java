...
public class MySQL5BrokerDAO extends BrokerDAO {
			...
			public void handleRead(ResultSet rset) throws SQLException {
				while (rset.next()) {
					...
					brokerItems.add(new Item(itemUniqueId, itemId, itemCount, itemColor, colorExpireTime, itemCreator, expireTime,
						...
						optionalFusionSocket, charge, randomBonus, rndCount));
				}
			}
		});
		return brokerItems;
	...
	private boolean insertBrokerItem(final BrokerItem item) {
		boolean result = DB.insertUpdate("INSERT INTO `broker` (`item_pointer`, `item_id`, `item_count`, `item_creator`, `seller`, `price`, `broker_race`, `expire_time`, `seller_id`, `is_sold`, `is_settled`) VALUES (?,?,?,?,?,?,?,?,?,?,?)", new IUStH() {
