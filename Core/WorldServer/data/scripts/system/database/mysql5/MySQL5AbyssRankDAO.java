...
public class MySQL5AbyssRankDAO extends AbyssRankDAO {
	...
	public static final String SELECT_PLAYERS_RANKING_ACTIVE_ONLY = "SELECT abyss_rank.rank, abyss_rank.ap, abyss_rank.old_rank_pos, abyss_rank.rank_pos, players.name, legions.name, players.id, players.title_id, players.player_class, players.exp FROM abyss_rank INNER JOIN players ON abyss_rank.player_id = players.id LEFT JOIN legion_members ON legion_members.player_id = players.id LEFT JOIN legions ON legions.id = legion_members.legion_id WHERE players.race = ? AND abyss_rank.ap > 0 AND UNIX_TIMESTAMP(CURDATE())-UNIX_TIMESTAMP(players.last_online) <= ? * 24 * 60 * 60 ORDER BY abyss_rank.ap DESC LIMIT 0, 300";
	...
	public AbyssRank loadAbyssRank(int playerId) {
		...
		try {
			...
			if (resultSet.next()) {
				...
				abyssRank = new AbyssRank(daily_ap, weekly_ap, ap, rank, top_ranking, daily_kill, weekly_kill, all_kill, max_rank, last_kill, last_ap, last_update);
	...
	public ArrayList<AbyssRankingResult> getAbyssRankingPlayers(final Race race, final int maxOfflineDays) {
		...
		try {
			...
			while (resultSet.next()) {
				...
				AbyssRankingResult rsl = new AbyssRankingResult(name, playerAbyssRank, playerId, ap, playerTitle, playerClass, playerLevel, playerLegion, oldRankPos, rankPos);
				results.add(rsl);
			}
			resultSet.close();
			stmt.close();
