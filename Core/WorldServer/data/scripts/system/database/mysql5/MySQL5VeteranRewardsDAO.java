...
public class MySQL5VeteranRewardsDAO extends VeteranRewardsDAO {
	...
	public Set<VeteranRewards> getVeteranReward() {
		...
		try {
			...
			while (rset.next()) {
				result.add(new VeteranRewards(rset.getInt("id"), rset.getString("player"), rset.getInt("type"), rset.getInt("item"), rset.getInt("count"), rset.getInt("kinah"), rset.getString("sender"), rset.getString("title"), rset.getString("message")));
			}
			rset.close();
			stmt.close();
		} catch (Exception e) {
